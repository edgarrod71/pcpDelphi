...
type
  TFN1_Refe_Presentacion = class(T_fvent_child)
...
var
  FN1_Refe_Presentacion: TFN1_Refe_Presentacion;

implementation

uses UN1_Refe_Presentacion1, _func_varias, URN1_Refe_Presentacion1;

{$R *.DFM}

{****************************************************************
Procedimiento   : padr_acti_nuevoExecute
Objetivo        : Crea el módulo presentación de variaciones y materiales
									de empaque. Se utiliza para los acti nuevo y modificar, no
                  hereda
Realizado por   : Gustavo Muñoz (DESARROLLO)
Fecha           : Miercoles 28 de Julio de 2004
Versión         : pcp4000
*****************************************************************}
procedure TFN1_Refe_Presentacion.padr_acti_nuevoExecute(Sender: TObject);
begin
//  inherited;
	//Verifico que no exista el módulo con el título
  If ExisteVentana('Presentación referencia ' + Consulta.FieldByName('NOMB_REFERENCIA').AsString +
    	' Variación ' + Consulta.FieldByName('NOMB_REFE_TIPO_VARIACION').AsString) Then
		Exit;
  //Creo el módulo
  FN1_Refe_Presentacion1 := TFN1_Refe_Presentacion1.Create(Self);
	With FN1_Refe_Presentacion1 Do
  Begin
  	Visible := False;
    //Asigno el título del módulo
    Caption := 'Presentación referencia ' + Consulta.FieldByName('NOMB_REFERENCIA').AsString +
    ...
    AbrirDataSet(QImagenes_Empaque);
...
Objetivo        : Crea el módulo de impresión de la presentación y
									Materiales de Empaque de la referencia - variación.
Realizado por   : Gustavo Muñoz (DESARROLLO)
Fecha           : Martes 27 de Julio de 2004
Versión         : pcp4000
*****************************************************************}
procedure TFN1_Refe_Presentacion.padr_acti_imprimirExecute(
  Sender: TObject);
Var
	I : Integer;
Const
	CosNoExistePresentacion = 'No existe especificaciones de presentación de la referencia %s variación %s';
begin
//  inherited;
	//Verifico que exista hoja de diseño de la referencia
	QExiste_Presentacion.Close;
  QExiste_Presentacion.Open;
  If QExiste_Presentacion.FieldByName('EXISTE').AsInteger = 0 Then
  Begin
    EjecutarMensaje(Format(CosNoExistePresentacion,
