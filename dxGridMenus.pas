{ 05/03/2004 10:41:46 a.m. (GMT-5:00) > [tecnologia on TECNOLOGIASCL] checked in \\  }
{ 05/03/2004 10:39:18 a.m. (GMT-5:00) > [tecnologia on TECNOLOGIASCL] checked out / }
unit dxGridMenus;
...
const
  StSortAscending = 'Orden &Ascendente';
  StSortDescending = 'Orden &Descendente';
  StGroupByThisColumn = 'A&grupar por esta columna';
  StRemoveThisColumn = 'Oculta&r esta columna';
  StColumnSelector = '&Selector de columnas...';
  StAlignment = '&Alineación del texto';
  StAlignments : Array[TAlignment] of String = ('&Izquierda', '&Derecha', '&Centrado');
  StBestFit = 'Ajustar';
  StBestFitAllColumns = 'Ajustar (Todas las columnas)';
  StSummaryItems: Array[TdxSummaryType] of String = ('Ninguno', 'Suma', 'Mínimo', 'Máximo', 'Contar', 'Promedio');
...
var
  Align : TAlignment;
begin
  Assert(fGridColumn <> nil, 'El parámetro para la columa está VACIO');
...
begin
  Assert(fGridColumn <> nil, 'El parámetro de la columna está VACIO');
  if IsRowFooter then
...
begin
  ...
  case Request of
    ...
    raise Exception.CreateFmt('Solicitud ilegal %d en AccessInstance',
      [Request]);
  end;
  Result := FInstance;
end;
...
begin
  inherited Create;
  raise Exception.CreateFmt('Sólo se permite Clase de Acceso %s a través de una Instancia',
    [ClassName]);
end;
