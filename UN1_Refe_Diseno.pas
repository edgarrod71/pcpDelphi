...
type
  TFN1_Refe_Diseno = class(T_fvent_child)
...
var
  FN1_Refe_Diseno: TFN1_Refe_Diseno;

implementation

uses UN1_Refe_Diseno1, _func_varias, URN1_Refe_Diseno1;

{$R *.DFM}

{****************************************************************
Procedimiento   : padr_acti_nuevoExecute
Objetivo        : Crea el módulo hoja de diseño de referencias.
...
Versión         : pcp4000
*****************************************************************}
procedure TFN1_Refe_Diseno.padr_acti_nuevoExecute(Sender: TObject);
begin
//  inherited;
	//Verifico que no exista el módulo con el título
  If ExisteVentana('Diseño referencia ' + Consulta.FieldByName('NOMB_REFERENCIA').AsString +
    	' Variación ' + Consulta.FieldByName('NOMB_REFE_TIPO_VARIACION').AsString) Then
		Exit;
  //Creo el módulo
  FN1_Refe_Diseno1 := TFN1_Refe_Diseno1.Create(Self);
	With FN1_Refe_Diseno1 Do
  Begin
  	Visible := False;
...
Objetivo        : Crea el módulo de impresión hoja de diseño de referencias.
Realizado por   : Gustavo Muñoz (DESARROLLO)
Fecha           : Martes 27 de Julio de 2004
Versión         : pcp4000
*****************************************************************}
procedure TFN1_Refe_Diseno.padr_acti_imprimirExecute(Sender: TObject);
Var
	I : Integer;
Const
	CosNoExisteDiseno = 'No existe especificaciones de diseño de la referencia %s variación %s';
begin
//  inherited;
	//Verifico que exista hoja de diseño de la referencia
	QExiste_Diseno.Close;
  QExiste_Diseno.Open;
  If QExiste_Diseno.FieldByName('EXISTE').AsInteger = 0 Then
  Begin
