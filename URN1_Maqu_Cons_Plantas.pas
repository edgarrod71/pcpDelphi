...
uses
  ...
  PCPQrDBText, QrAngLbl, JvExControls, JvComponent, JvGradientHeaderPanel,
  SCLTabs;

type
  TRN1_Maqu_Cons_Plantas = class(TFInformes)
    QPlantas: TQuery;
  ...
  public
    ...
    procedure Total_Familia(Memo : TQRMemo; Posicion : Integer);
    procedure Tot_Seccion(Memo : TQRMemo);
  end;

var
  RN1_Maqu_Cons_Plantas: TRN1_Maqu_Cons_Plantas;
...
Versión         : pcp4000
*****************************************************************}
procedure TRN1_Maqu_Cons_Plantas.Total_Familia(Memo : TQRMemo; Posicion : Integer);
Var
	I, Total : Integer;
begin
	Total := 0;
...
Versión         : pcp4000
*****************************************************************}
procedure TRN1_Maqu_Cons_Plantas.Tot_Seccion(Memo : TQRMemo);
Var
	I, Total : Integer;
begin
	Total := 0;
...
Objetivo        : Arma la matriz con los memos de las cantidades
									por planta, sección y familia de recursos
Realizado por   : Gustavo Muñoz(DESARROLLO)
Fecha           : Viernes 23 de Julio de 2004
Versión         : pcp4000
*****************************************************************}
procedure TRN1_Maqu_Cons_Plantas._QRBanda_MaestraBeforePrint(
...
begin
  inherited;
	Inc(Total_Plantas);
	//Veirifico que la planta tenga asignado Recusros
	If QCantidad.Locate('CONS_PLANTA', QPlantas.FieldByName('CONS_PLANTA').AsInteger, []) Then
  	PrintBand := True
  Else
  	PrintBand := False;
  //La planta tiene asignado Recursos
	If PrintBand Then
  Begin
    ...
    For I := 1 To 17 Do
    Begin
      //Asigno el memo que corresponde a la familia
    ...
    Begin
      ...
      MSecciones.Lines.Add('  ' + QSecciones.FieldByName('NOMB_SECCION').AsString);
			Total_Seccion := 0;
      //Busco las 13 posibles familias
      For I := 1 To 17 Do
      Begin
        //Asigno el memo que corresponde a la familia
				Memo := _QRBanda_Maestra.FindComponent('TQRMemo_' + IntToStr(I));
        If Memo <> Nil Then
        Begin
          //Asigno el titulo que corresponde a la familia
          Titulo := _QRBanda_Page_Header.FindComponent('TPCPQRLabel_' + IntToStr(I));
          If Titulo <> Nil Then
            //Busco si la planta en la sección y familia tiene Recursos asignados
          	If QCantidad.Locate('CONS_PLANTA;CONS_SECCION;CODI_RECU_FAMILIA',
            ...
            Begin
              //Adiciono la cantidad de recursos que estan asignados a la planta, sección, familia
            ...
            End
            Else
              //Si no tiene asignación de recursos
    ...
    Begin
      ...
      Memo := _QRBanda_Maestra.FindComponent('TQRMemo_' + IntToStr(I));
      If Memo <> Nil Then
        Total_Familia((Memo As TQRMemo), I);
    End;
    Memo := _QRBanda_Maestra.FindComponent('TQRMemo_Total');
    If Memo <> Nil Then
...
Versión         : pcp4000
*****************************************************************}
procedure TRN1_Maqu_Cons_Plantas.LPLantaPrint(sender: TObject;
  var Value: String);
begin
  inherited;
	Value := ' ' + Value;
...
Versión         : pcp4000
*****************************************************************}
procedure TRN1_Maqu_Cons_Plantas._QRPadreBeforePrint(
...
Versión         : pcp4000
*****************************************************************}
procedure TRN1_Maqu_Cons_Plantas.BTotalesBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
Var
	I : Integer;
  X : TComponent;
begin
  ...
  Begin
  	PrintBand := True;
    //Recorro la cantidad de familia de recursos y le asigno el total
    For I := 1 To 17 Do
    Begin
...
procedure TRN1_Maqu_Cons_Plantas.FormCreate(Sender: TObject);
begin
  inherited;
